import torch
import torch.nn as nn
from einops import rearrange
from einops.layers.torch import Rearrange
import random


class DropPath(nn.Module):
    def __init__(self, drop_prob: float, scale_by_keep: bool = True):
        super(DropPath, self).__init__()
        self.drop_prob = drop_prob
        self.scale_by_keep = scale_by_keep

    def forward(self, x):
        if self.drop_prob <= 0. or not self.training:
            return x
        keep_prob = 1 - self.drop_prob
        shape = (x.shape[0],) + (1,) * (x.ndim - 1)  # work with diff dim tensors, not just 2D ConvNets
        random_tensor = x.new_empty(shape).bernoulli_(keep_prob)
        if keep_prob > 0.0 and self.scale_by_keep:
            random_tensor.div_(keep_prob)
        return x * random_tensor


class PreNorm(nn.Module):
    def __init__(self,
                 dim: int,
                 fn: nn.Module):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.fn = fn

    def forward(self, x, **kwargs):
        return self.fn(self.norm(x), **kwargs)


class FeedForward(nn.Module):
    def __init__(self,
                 input_dim: int,
                 output_dim: int,
                 hidden_dim: int,
                 drop_out_rate=0.):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(input_dim, hidden_dim),
                                 nn.GELU(),
                                 nn.Dropout(drop_out_rate),
                                 nn.Linear(hidden_dim, output_dim),
                                 nn.Dropout(drop_out_rate))

    def forward(self, x):
        return self.net(x)


class Attention(nn.Module):
    def __init__(self,
                 input_dim: int,
                 output_dim: int,
                 heads: int = 8,
                 dim_head: int = 64,
                 qkv_bias: bool = True,
                 drop_out_rate: float = 0.,
                 attn_drop_out_rate: float = 0.):
        super().__init__()
        inner_dim = dim_head * heads
        project_out = not (heads == 1 and dim_head == input_dim)

        self.heads = heads
        self.scale = dim_head ** -0.5

        self.attend = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(attn_drop_out_rate)
        self.to_qkv = nn.Linear(input_dim, inner_dim * 3, bias=qkv_bias)

        if project_out:
            self.to_out = nn.Sequential(nn.Linear(inner_dim, output_dim),
                                        nn.Dropout(drop_out_rate))
        else:
            self.to_out = nn.Identity()

    def forward(self, x):
        qkv = self.to_qkv(x).chunk(3, dim=-1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.heads), qkv)

        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale

        attn = self.attend(dots)
        attn = self.dropout(attn)
        out = torch.matmul(attn, v)
        out = rearrange(out, 'b h n d -> b n (h d)')
        out = self.to_out(out)
        return out


class TransformerBlock(nn.Module):
    def __init__(self,
                 input_dim: int,
                 output_dim: int,
                 hidden_dim: int,
                 heads: int = 8,
                 dim_head: int = 32,
                 qkv_bias: bool = True,
                 drop_out_rate: float = 0.,
                 attn_drop_out_rate: float = 0.,
                 drop_path_rate: float = 0.):
        super().__init__()
        attn = Attention(input_dim=input_dim,
                         output_dim=output_dim,
                         heads=heads,
                         dim_head=dim_head,
                         qkv_bias=qkv_bias,
                         drop_out_rate=drop_out_rate,
                         attn_drop_out_rate=attn_drop_out_rate)
        self.attn = PreNorm(dim=input_dim,
                            fn=attn)
        self.droppath1 = DropPath(drop_path_rate) if drop_path_rate > 0 else nn.Identity()

        ff = FeedForward(input_dim=output_dim,
                         output_dim=output_dim,
                         hidden_dim=hidden_dim,
                         drop_out_rate=drop_out_rate)
        self.ff = PreNorm(dim=output_dim,
                          fn=ff)
        self.droppath2 = DropPath(drop_path_rate) if drop_path_rate > 0 else nn.Identity()

    def forward(self, x):
        x = self.droppath1(self.attn(x)) + x
        x = self.droppath2(self.ff(x)) + x
        return x


class ViT(nn.Module):
    def __init__(self,
                 num_leads: int,
                 seq_len: int,
                 patch_size: int,
                 width: int = 768,
                 depth: int = 12,
                 mlp_dim: int = 3072,
                 heads: int = 12,
                 dim_head: int = 64,
                 qkv_bias: bool = True,
                 drop_out_rate: float = 0.,
                 attn_drop_out_rate: float = 0.,
                 drop_path_rate: float = 0.,
                 use_multilayer_selection: bool = False,
                 **kwargs):
        super().__init__()
        assert seq_len % patch_size == 0, 'The sequence length must be divisible by the patch size.'
        self.num_patches = seq_len // patch_size
        
        # original patch start
        self.to_patch_embedding = nn.Sequential(Rearrange('b c (n p) -> b n (p c)', p=patch_size),
                                                nn.LayerNorm(patch_size),
                                                nn.Linear(patch_size, width),
                                                nn.LayerNorm(width))

        # w/o cls token
        self.pos_embedding = nn.Parameter(torch.randn(1, self.num_patches, width))

        # define unique lead embeddings for each lead (12 leads)
        self.lead_embeddings = nn.ParameterList(nn.Parameter(torch.randn(width))
                                                for _ in range(num_leads))

        # w cls token
        # self.pos_embedding = nn.Parameter(torch.randn(1, self.num_patches+1, width))
        # self.cls_token = nn.Parameter(torch.randn(1, 1, width))
        
        self.dropout = nn.Dropout(drop_out_rate)

        
        self.depth = depth
        self.width = width
        self.use_multilayer_selection = use_multilayer_selection
        
        # 多层特征选择策略：Beginning (第2层/Index 1), Middle (第6层/Index 5), Ending (第12层/Index 11)
        if self.use_multilayer_selection:
            # 对于12层ViT-Tiny，选择第1、5、11层（0-indexed）
            self.selected_layers = [1, 5, 11]  # Beginning, Middle, Ending stages
            # 融合方式：'concat' (拼接) 或 'add' (相加)
            # 从kwargs中获取fusion_method，默认为'concat'
            self.fusion_method = kwargs.get('fusion_method', 'concat')
            
            if self.fusion_method == 'concat':
                # 外部直接融合：拼接三层特征后投影回原始维度
                # 输入维度: width * 3, 输出维度: width
                self.fusion_projection = nn.Linear(width * 3, width)
            elif self.fusion_method == 'add':
                # 相加融合：三层特征相加后可选投影
                # 相加后维度仍为 width，可选投影层用于特征调整
                self.fusion_projection = nn.Linear(width, width)
            else:
                raise ValueError(f"Unsupported fusion_method: {self.fusion_method}. Must be 'concat' or 'add'.")
        else:
            self.selected_layers = []
            self.fusion_projection = None
            self.fusion_method = None
        
        drop_path_rate_list = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
        for i in range(depth):
            block = TransformerBlock(input_dim=width,
                                     output_dim=width,
                                     hidden_dim=mlp_dim,
                                     heads=heads,
                                     dim_head=dim_head,
                                     qkv_bias=qkv_bias,
                                     drop_out_rate=drop_out_rate,
                                     attn_drop_out_rate=attn_drop_out_rate,
                                     drop_path_rate=drop_path_rate_list[i])
            self.add_module(f'block{i}', block)

        self.norm = nn.LayerNorm(width)
        self.head = nn.Identity()

    
    def forward_encoding(self, series, pool='mean'):

        # for original patch start
        L = series.shape[1] # series -> [batch_size, num_leads, seq_len]
        if L > len(self.lead_embeddings):
            raise ValueError(f'Number of leads must be less than or equal to {self.num_leads}')

        all_leads = []
        for i in range(L):
            # patchify each lead
            x_lead = self.to_patch_embedding(series[:, i:i + 1, :])

            # add unique lead embeddings
            lead_embedding = self.lead_embeddings[i].unsqueeze(0)
            lead_embedding = lead_embedding.expand(x_lead.shape[0], x_lead.shape[1], -1)
            x_lead += lead_embedding

            # w cls token
            # x_lead = torch.cat([self.cls_token.expand(x_lead.shape[0], -1, -1), x_lead], dim=1)

            # add positional embeddings (shared across leads)
            x_lead = x_lead + self.pos_embedding

            x_lead = x_lead.unsqueeze(1)
            all_leads.append(x_lead)

        x = torch.cat(all_leads, dim=1) # x -> [batch_size, num_leads, num_patches, embed_dim]

        # w/o iterative process lead-wise
        # x = torch.cat(x, dim=1)
        x = torch.cat([x[:, i] for i in range(x.size(1))], dim=1)   # x -> [batch_size, num_leads*num_patches, embed_dim]

        # 多层特征选择策略
        if self.use_multilayer_selection:
            selected_features = []
            for i in range(self.depth):
                x = getattr(self, f'block{i}')(x)
                # 保存选定层的特征
                if i in self.selected_layers:
                    selected_features.append(x)
            
            # 融合方式选择：拼接或相加
            if self.fusion_method == 'concat':
                # 外部直接融合：在通道维度拼接三层特征
                # selected_features: [feat1, feat2, feat3], 每个形状为 [batch_size, num_patches, width]
                # 拼接后: [batch_size, num_patches, width * 3]
                fused_features = torch.cat(selected_features, dim=-1)
                # 投影回原始维度: [batch_size, num_patches, width]
                x = self.fusion_projection(fused_features)
            elif self.fusion_method == 'add':
                # 相加融合：三层特征直接相加
                # selected_features: [feat1, feat2, feat3], 每个形状为 [batch_size, num_patches, width]
                # 相加后: [batch_size, num_patches, width]
                fused_features = sum(selected_features)  # 或使用 torch.stack + sum
                # 可选投影层用于特征调整
                x = self.fusion_projection(fused_features)
        else:
            # 原始方式：只使用最后一层
            for i in range(self.depth):
                x = getattr(self, f'block{i}')(x)

        # extract cls and avg pool
        # pool_x = [x[:, i, :].unsqueeze(1) for i in range(0, x.shape[1], self.num_patches+1)]
        # pool_x = torch.cat(pool_x, dim=1)
        # pool_x = torch.mean(pool_x, dim=1)
        
        if  pool == 'none':
            return x
        elif pool == 'cls':
            raise NotImplementedError('CLS token pooling is not supported for lead-wise input.')
        elif pool == 'mean':
            # x = torch.mean(x, dim=1) 
            pass

        # return self.norm(x), self.norm(pool_x)
        return self.norm(x)
    
    def forward(self, series, pool='mean'):
        x = self.forward_encoding(series, pool=pool)
        x = self.head(x)
        return x

    def reset_head(self, num_classes=1):
        del self.head
        self.head = nn.Linear(self.width, num_classes)


def vit_tiny(num_leads, num_classes=1, seq_len=5000, patch_size=100, **kwargs):
    model_args = dict(num_leads=num_leads,
                      num_classes=num_classes,
                      seq_len=seq_len,
                      patch_size=patch_size,
                      width=192,
                      depth=12,
                      heads=3,
                      mlp_dim=768,
                      **kwargs)
    return ViT(**model_args)


def vit_small(num_leads, num_classes=1, seq_len=5000, patch_size=100, **kwargs):
    model_args = dict(num_leads=num_leads,
                      num_classes=num_classes,
                      seq_len=seq_len,
                      patch_size=patch_size,
                      width=384,
                      depth=12,
                      heads=6,
                      mlp_dim=1536,
                      **kwargs)
    return ViT(**model_args)


def vit_middle(num_leads, num_classes=1, seq_len=5000, patch_size=100, **kwargs):
    model_args = dict(num_leads=num_leads,
                      num_classes=num_classes,
                      seq_len=seq_len,
                      patch_size=patch_size,
                      width=512,
                      depth=12,
                      heads=8,
                      mlp_dim=2048,
                      **kwargs)
    return ViT(**model_args)


def vit_base(num_leads, num_classes=1, seq_len=5000, patch_size=100, **kwargs):
    model_args = dict(num_leads=num_leads,
                      num_classes=num_classes,
                      seq_len=seq_len,
                      patch_size=patch_size,
                      width=768,
                      depth=12,
                      heads=12,
                      mlp_dim=3072,
                      **kwargs)
    return ViT(**model_args)
